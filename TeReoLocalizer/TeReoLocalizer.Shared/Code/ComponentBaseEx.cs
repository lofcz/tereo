...
public abstract class ComponentBaseInternal : IComponent, IHandleEvent, IHandleAfterRender
{
    ...
    /// <summary>
    /// A slightly faster version of <see cref="StateHasChanged"/> that can be called only from UI thread.
    /// </summary>
    public void StateHasChangedUiThread()
    {
    ...
    public void StateHasChanged()
    {
        ...
        {
            InvokeAsync(StateHasChangedInternal);
            return;
        }
    ...
    {
        // This implicitly means a ComponentBaseInternal can only be associated with a single
        ...
        {
            throw new InvalidOperationException($"The render handle is already set. Cannot initialize a {nameof(ComponentBaseInternal)} more than once.");
        }
    ...
    private async Task RunInitAndSetParametersAsync()
    {
        ...
        Task blockingTask = OnInitializedAsyncBlocking();

        if (blockingTask.Status is not (TaskStatus.RanToCompletion or TaskStatus.Canceled))
        {
            try
            {
                await blockingTask;
...
public class ComponentBaseEx : ComponentBaseInternal
{
    [Parameter]
